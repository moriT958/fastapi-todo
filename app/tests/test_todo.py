...
def test_create_todo(client_fixture: TestClient):
    response = client_fixture.post("/todos", json={"title": "running", "user_id": 1})
...
def test_update_todo_正常系(client_fixture: TestClient):
    response = client_fixture.patch("/todos/1", json={"status": "COMPLETED"})
    assert response.status_code == 200
...
def test_update_todo_異常系(client_fixture: TestClient):
    response = client_fixture.patch("/todos/10", json={"status": "COMPLETED"})
    assert response.status_code == 404
