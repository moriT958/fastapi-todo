...
def update_todo(db: Session, id: int, todo_update: TodoUpdate) -> Optional[Todo]:
    ...
    if update_todo is None:
        return None
    updating_todo.title = updating_todo.title if update_todo.title is None else update_todo.title
    updating_todo.status = updating_todo.status if update_todo.status is None else update_todo.status
    db.add()
    db.commit()
