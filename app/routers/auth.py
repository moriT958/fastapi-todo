...
async def login_user(db: dbDep, form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    user = auth_cruds.authenticate_user(db, form_data.username, form_data.password)
    if not user:
        raise HTTPException(status_code=401, detail="username or password is incorrect.")

    token = auth_cruds.create_access_token(user.username, user.id, timedelta(minutes=15))
    return {
        "access_token": token,
        "token_type": "bearer"
    }